              ...
              <v-col cols="12" sm="10" :class="compact ? '' : 'pr-2'">
                <v-row class="align-center" dense :no-gutters="compact">
                  ...
                  <v-col cols="12" :sm="4" :class="compact ? 'pr-2' : ''">
                    ...
                    <!-- INPUT FECHA -->
                    <v-no-ssr>
                      <VueDatePicker class="search-datepicker-input" :class="compact ? '' : ' mt-2'" ref="dp"
                        v-model="date" range multi-calendars :teleport="true" :enable-time-picker="false" locale="es"
                        :min-date="new Date()">
                        <template #action-buttons="{ value }">
                          <v-btn variant="flat" rounded="md" color="primary" @click="selectDate">
...
async function searchResults() {
  const query = {
    arrivalDate: dayjs(date.value[0]).format('YYYY-MM-DD'),
    departureDate: dayjs(date.value[1]).format('YYYY-MM-DD'),
...
onMounted(() => {
  if (route.query.arrivalDate) {
    date.value[0] = route.query.arrivalDate
  }
  if (route.query.departureDate) {
    date.value[1] = route.query.departureDate
  }
