<template>
    ...
    <v-row>
        ...
        <v-col cols="12" md="7" v-if="can('Sellers', 'Division')"> 
            <v-autocomplete label="Agencia" variant="outlined" density="compact" :items="clientsStore.getClients" v-model="clientSelected" v-model:search="clientsSearch"
                item-title="FantasyName" return-object @change="fetchSellersByClient()"  clearable
                no-data-text="No hay resultados para su búsqueda">
                <template v-slot:append-inner>
                    <v-progress-circular indeterminate color="primary" size="25"
            ...
            <v-autocomplete label="Agente" variant="outlined" density="compact" :items="sellers"
                v-model="clientSellerSelected" item-title="Value" return-object :disabled="!clientsStore.getClients || clientsStore.getClients.length == 0"
                no-data-text="No hay resultados para su búsqueda">
                <template v-slot:append-inner>
                    <v-progress-circular indeterminate color="primary" size="25"
                        v-if="loadingSellers"></v-progress-circular>
        ...
        <v-col v-if="can('Sellers', 'Division')"> 
            <v-card variant="outlined" class="pa-2 bg-error_lighten error" rounded="lg" v-if="!clientSellerSelected">
                <h4 class="text-error text-h6 semi"><v-icon icon="mdi-alert-circle" class="mr-2"></v-icon>Atención</h4>
                <p class="body-2 pa-4">Debe seleccionar una agencia cliente. De lo contrario, la actividad será auditada
                    y reportada como mala práctica. Si continúa con este procedimiento será punible de penalidades.</p>
            </v-card>
        </v-col>
    </v-row>
</template>
...
async function changeClientSeller() {
    ...
    let payload = {
        agencyId: clientSelected.value.LegacyId,
    ...
    try {
        const result = await useNuxtApp().$toast.promise(clientsStore.changeClientSeller(payload), {
            error: "Error al setear el vendedor minorista",
        });
