  ...
  <v-dialog v-model="show" width="600px" radius="xl" >
    <v-card rounded="lg" class="bg-midground">
      <v-card-text class="pa-6">
        <h4 class="text-h5 text-primary_text text-center" v-if="!bookingConfirmed">Hemos recibido su solicitud. <br>Estamos contactando
        ...
        <v-alert size="large" class="text-center" color="success" variant="tonal" border="start"  v-if="!waitingProvider && bookingConfirmed">
        ...
        <stays-checkout-details-card :booking="booking" :serviceType="serviceType" class="mt-5"></stays-checkout-details-card>
        <v-card class="mt-4" flat v-for="(service, index) in booking.Services" v-if="!waitingProvider && bookingConfirmed"
         >
        ...
        <v-row dense class="mt-5" v-if="!waitingProvider && bookingConfirmed">
          <v-col><v-btn flat block color="primary" rounded="xl" @click="$emit('back')">Volver a la
              búsqueda</v-btn></v-col>
