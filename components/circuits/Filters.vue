<template>
    <v-row dense class="mb-1" v-if="filters.length > 0">
        <v-col cols="12">
            <v-btn @click="applyFilters" density="comfortable" rounded="sm" class="mr-1 semi body-2" flat
            ...
            <v-menu center bottom offset-y transition="scale-transition" :close-on-content-click="false" rounded
                ...
                <template v-slot:activator="{ props }">
                    <v-btn class="mr-1 px-3 my-1"
                        ...
                        density="comfortable" :variant="checkFeatureChecked(item) ? 'tonal' : 'outlined'" rounded="sm"
                        v-bind="props" :color="checkFeatureChecked(item) ? 'secondary' : 'secondary_darken'"
                        ...
                        <v-checkbox v-model="feature.Checked" density="compact" hide-details
                            ...
                            <template v-slot:label>
                                <v-rating size="small" density="compact" v-model="feature.Value" readonly
                                    half-increments v-if="item.GroupName == 'feature.propertyCategory'"></v-rating>
