<template>
    <v-row dense class="mb-1" v-if="filters.length > 0">
        <v-col cols="12">
            <v-btn @click="applyFilters" density="comfortable" rounded="sm" class="mr-1 semi body-2"
                flat color="secondary" :disabled="!hasActiveFilters">Aplicar filtros</v-btn>
            <v-menu center bottom offset-y transition="scale-transition" :close-on-content-click="false" rounded
                ...
                <template v-slot:activator="{ props }">
                    <v-btn class="mr-1 px-3 my-1"
                        ...
                        density="comfortable" :variant="checkFeatureChecked(item) ? 'tonal' : 'outlined'" rounded="sm" v-bind="props"
            ...
            <v-btn class="ml-1 px-3 my-1 ml-auto" color="secondary_text" density="comfortable" rounded="sm" variant="text"
