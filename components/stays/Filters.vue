<template>
  ...
  <v-row dense class="mb-1" v-if="filters.length > 1">
    <v-col cols="12">
      <v-icon icon="mdi-tune" size="small" v-if="!compact"></v-icon> <span class="body-1 mr-2" v-if="!compact">Filtros</span>
      <v-menu center bottom offset-y transition="scale-transition" :close-on-content-click="false" rounded  :attach="true" 
        ...
        <div class="bg-background pa-2 rounded-lg border-secondary_lighten">
          <v-list-item v-for="feature in item.Features" class="bg-background" >
            <v-checkbox v-model="feature.Checked" density="compact" hide-details @change="updateSelectedFilters(item.GroupName, feature)">
              <template v-slot:label>
                <div v-if="item.GroupName == 'feature.propertyCategory'">
                  <v-rating size="small" density="compact" v-model="feature.Value" readonly half-increments></v-rating>
      ...
      <v-btn v-if="hasCheckedFilters" @click="clearFilters" color="secondary_text" dark size="small" rounded="sm" variant="text" >
