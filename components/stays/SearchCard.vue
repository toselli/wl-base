    ...
    <v-row dense no-gutters :class="noresults ? 'justify-center' : ''">
        ...
        <v-card :rounded="compact ? 'md' : ''"
          ...
          <v-row class="align-center justify-space-between" dense>
            ...
            <v-col cols="12" :sm="noplaces ? '7' : '4'" :class="isMobile ? 'mt-3' : 'pr-2'">
              ...
              <!-- INPUT FECHA -->
              <CommonDatePicker :compact="compact" :searchedDate="selectedDate" @update:selectedDate="handleSelectedDate" />
            </v-col>
      ...
      <v-col cols="12" sm="2" class="text-center" :class="isMobile ? 'mt-1' : ''">
        <v-btn rounded="md" color="primary" :variant="compact ? 'tonal' : 'flat'"
          :class="compact ? 'semi mt-1' : 'semi mt-2'" v-if="noplaces" :block="isMobile"
          :size="compact ? 'large' : 'x-large'" prepend-icon="mdi-magnify" @click="searchResults"
          :disabled="selectedDate == []">
          {{ $capitalize($t("search")) }}
        </v-btn>
        <v-btn rounded="md" color="primary" :variant="compact ? 'tonal' : 'flat'"
          :class="compact ? 'semi' : 'semi mt-3'" v-else :block="isMobile" :size="compact ? 'large' : 'x-large'"
