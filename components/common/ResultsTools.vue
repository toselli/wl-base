        ...
        <v-menu>
            <template v-slot:activator="{ props }">
                <v-btn variant="flat" density="comfortable" class="mr-2 text-secondary_darken" v-bind="props"
                    rounded="md">
                    <v-icon icon="mdi-sort" class="mr-1"></v-icon> Ordenar por <span class="body-3 ml-1">{{
        selectedOrderBy
    }}</span>
                </v-btn>
            </template>
            <v-list flat color="foreground">
                <v-list-item v-for="item in sorting" class="body-1" @click="() => selectOrderBy(item)">
                    {{ item?.name }} <v-icon size="16">{{ item?.icon }}</v-icon>
                </v-list-item>
            </v-list>
        </v-menu>
        <v-btn-toggle v-model="viewModeModel" density="compact" mandatory color="secondary" class="bg-foreground pa-1"
            variant="flat" selected-class="bg-secondary_darken text-white" rounded="md" v-if="!isMobile && serviceType != 'flights'">
...
const props = defineProps(["viewMode", "serviceType", "showMap", "loading", "sorting", "orderType"]);
...
const selectOrderBy = (item) => {
    selectedOrderBy.value = item.name;
    emit('update:order', {
        name: item.name,
        field: item.field,
...
watch(props, (newValue, oldValue) => {
    selectedOrderBy.value = props.orderType.name
},
    { deep: true });
