    ...
    <v-container class="content-container">
        ...
        <v-row class="py-2" v-if="getLoggedUser">
            <v-col cols="12">
                ...
                <flights-filters :filters="flightsStore.features" @apply="getFilteredResults"
                    @clear="clearFilters"></flights-filters>
                <p class="body-2 text-secondary_text" v-if="loading">
                    Buscando...
                </p>
                <h4 class="semi mb-2" v-if="!loading">
                ...
                <v-btn-toggle v-if="flex && flightsStore.getMatrix && flightsStore.getMatrix.length"
        ...
        <v-navigation-drawer temporary class="rates-nav-drawer" v-model="rateDialog" location="right">
            ...
            serviceSelected.length }}</span>
                ...
                <v-slide-group mandatory  show-arrows
                    class="d-flex justify-center fill-height slide-group">
                    ...
                    <template v-slot:next>
                        <v-btn icon :disabled="slideNumber == serviceSelected.length - 1" class="bg-transparent" variant="flat" elevation="0" @click="slideNumber++">
                            <v-icon>mdi-chevron-right</v-icon>
                        </v-btn>
                    </template>
                    <template v-slot:prev>
                        <v-btn icon :disabled="slideNumber == 0" class="bg-transparent" variant="flat" elevation="0" @click="slideNumber--">
...
const search = async (newRoute?: RouteLocationNormalized) => {
    ...
    const flex = route.query.flex === 'true' ? true : false
    const search = {
    ...
    try {
        await useNuxtApp().$toast.promise(flightsStore.search(payload, flex), {
...
onMounted(() => {
    flex.value = route.query.flex === 'true' ? true : false
