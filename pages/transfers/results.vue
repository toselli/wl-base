                    ...
                    <v-container class="content-container" fluid >
                        <v-row dense>
                            <v-col cols="12">
                                <v-row dense justify="center">
                                    <v-col :lg="viewMode == 'list' ? 12 : 6" cols="12"
                                    ...
                                    <v-col cols="12" class="text-center" v-if="!loading">
                                        <v-btn variant="outlined" rounded="xl" @click="viewMoreResultsIn"
                                            v-if="filteredResults?.transfersIn.length" :loading="loadingMore"> limitResultsIn">Mostrar más
                                            resultados</v-btn>
                                    </v-col>
                                </v-row>
                            </v-col>
                        ...
                        <v-row class="mt-3" justify="center" v-if="filteredResults?.transfersIn?.length == 0 && !loading">
                            <v-col cols="12" class="text-center">
                                ...
                                <v-img src="/img/no_results_transfers.png" width="280" class="my-10 mx-auto"></v-img>
                ...
                <v-window-item :value="2" v-if="filteredResults?.transfersOut?.length > 1">
                    <v-container class="content-container" fluid>
                        ...
                        <v-row class="mt-3" justify="center" v-if="filteredResults?.transfersOut?.length == 0 && !loading">
                            <v-col cols="12" class="text-center">
                                <h3>No hemos podido encontrar traslados con los parámetros de tu búsqueda</h3>
                                <v-img src="/img/no_results_transfers.png" width="280" class="mx-auto"></v-img>
                                <h4 class="body-1 semi text-secondary_text  mt-5">Pruebe realizando otra búsqueda</h4>
                                <transfers-search-card :compact="true" :noresults="true" class="mx-auto mt-2" />
                            </v-col>
                        </v-row>
            ...
            <v-card rounded="lg" class="bg-midground">
                <v-card-text class="pa-6">
                    ...
                    <v-row dense class="mt-5">
                        <v-col><v-btn variant="flat" color="primary" block rounded="xl" @click="reloadResults()">Comprobar
                                nueva
...
watch(getLoggedUser, (newValue, oldValue) => {
    if (newValue != null) {
        getResults();
        startCountdown()
    }
